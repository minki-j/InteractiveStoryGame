...
from app.agents.state_schema import OverallState, OutputState, Outline
...
def router(state: OverallState):
    print("/n>>> EDGE: router")
    return [
        Send(n(examine_each_unit_of_outline), {"unit": unit})
        for field in state.outline.__annotations__
        for unit in getattr(state.outline, field)
    ]


def examine_each_unit_of_outline(state: Unit):
    print(state["unit"])
    return 
